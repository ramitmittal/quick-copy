...
(async function loadData() {
  ...
  if (
    result.extData !== null &&
    result.extData.version === EXTENSION_DATA_VERSION
  ) {
...
async function updateStorage() {
  ...
  await browser.runtime
    .sendMessage({
      op: messageTypes.RESPONSE_CURRENT_COPYFIELDS,
      payload: copyFields,
    })
...
async function handleCommand(cmd) {
  ...
  if (
    ...
    const existingQuickSlot = Object.entries(copyFields).find(
      ([key, val]) => val.quickSlotNumber === quickSlotNumber
    );
...
async function handleMessage(msg) {
  ...
  if (op === messageTypes.EDIT_FROM_POPUP) {
    ...
    Object.entries(copyFields).forEach(([key, val]) => {
