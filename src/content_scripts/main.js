...
async function handleMessage (request) {
  ...
  if (op === messageTypes.COPY_FROM_PAGE) {
    ...
    const activeElemTagName = activeElement ? activeElement.tagName.toLowerCase() : null
  ...
  if (op === messageTypes.PASTE_TO_PAGE) {
    ...
    if (activeElemTagName === 'textarea' || activeElemTagName === 'input') {
      ...
      const effectiveText = activeElement.value.substring(0, startPos) + text + activeElement.value.substring(endPos)
